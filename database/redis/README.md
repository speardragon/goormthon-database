...
```yaml
namespace: goormthon-1 # 현재 팀 네임스페이스로 변경 <예: goormthon-1>
```
...
```yaml
kubectl delete pvc -l app=redis -n goormthon-1
```
...
```
redis-0.redis.goormthon-1.svc.cluster.local:6379
```
...
1. `base/kustomization.yaml` 파일에서 `goormthon-1`을 실제 네임스페이스로 바꿉니다.
